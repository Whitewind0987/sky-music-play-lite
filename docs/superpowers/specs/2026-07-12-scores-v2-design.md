...
- `src/types/score.ts`:`Note` 增加可选 `duration?: number`(毫秒),`Song` 增加可选 `formatVersion?: 1 | 2`。
- 内部保留显式版本标记；缺省仍表示 v1。这样 v2 曲谱经过规范化、托管存储、复制和重新导入后不会丢失时值语义。

### 导入文件存储(Rust `imported_scores.rs`)

- `save_imported_score_song` 写入规范化歌曲；包含有效时值或源歌曲显式为 v2 时,托管文件保留 `formatVersion: 2`。
- 早期开发版本生成的“含 `duration` 但缺少版本标记”托管文件在正常存储迁移中按 v2 安全升级,仍使用既有原子替换与文件保护流程。
- `SemanticNote` 增加 `duration: Option<f64>`,使保存后回读的内容语义校验覆盖时值(防止时值损坏未被发现)。
- 托管文件轻量校验:音符若含 `duration` 字段,必须是有限正数且不超过 60000ms。

### 本地曲目元数据(`LocalSongMetadata`)

- 保留可选兼容字段 `sustainTailMs`,并增加与 `noteGroupDelaysMs` 对齐的 `noteGroupMaxHoldMs?: number[]`;每项是该组最大显式时值,无时值为 0。
- 用于不加载全曲时的时长显示(曲库列表)。
...
- `getAdjustedPreviewDurationMs` / 总时长:逐组计算 `adjusted_start_i + max_hold_i / playbackSpeed`,再与最后组起点取最大值；正负 `noteIntervalDelayMs` 只影响组起点,不改变音乐时值本身。
- `getAdjustedPreviewDurationFromMetadata`:在分组延迟和分组最大时值数组都有效时复现同一精确公式；旧元数据继续使用 `sustainTailMs` 安全回退。
